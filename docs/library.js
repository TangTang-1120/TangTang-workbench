...
function videoCardHtml(e) {
  ...
  const id = e.id || "";
  const poster = e.posterUrl || (e.hasPoster ? `/TangTang-workbench/gallery/${id}/poster.jpg` : "");
  const videoSrc = e.videoUrl || e.solfegeUrl || `/TangTang-workbench/gallery/${id}/solfege.mp4`;
  const dlSol =
    ...
    if (isStaticSite()) {
      ...
      listEl.innerHTML = `
        <article class="library-row">
          ...
          <a class="btn-dl library-dl" href="/TangTang-workbench/">回首页</a>
        </article>`;
      return;
    }
